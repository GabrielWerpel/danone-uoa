...
def get_or_insert_product(product_name, product_code=None):
    logger.debug(f"Looking up ProductID for ProductName: {product_name}, ProductCode: {product_code}")
    cursor.execute("SELECT ProductID FROM Product WHERE ProductName = ?", product_name)
    row = cursor.fetchone()
    if row:
        logger.debug(f"Found existing ProductID: {row[0]} for ProductName: {product_name}")
        return row[0]
    try:
        cursor.execute("INSERT INTO Product (ProductName, ProductCode) VALUES (?, ?)", product_name, product_code)
        cursor.execute("SELECT SCOPE_IDENTITY()")
        product_id_row = cursor.fetchone()
        product_id = product_id_row[0] if product_id_row else None
        logger.debug(f"Inserted new ProductID: {product_id} for ProductName: {product_name}")
        return product_id
    except Exception as e:
        logger.error(f"Failed to insert/select ProductID for ProductName '{product_name}': {e}")
        return None

def get_or_insert_operator(operator_name):
    logger.debug(f"Looking up OperatorID for OperatorName: {operator_name}")
    cursor.execute("SELECT OperatorID FROM Operator WHERE OperatorName = ?", operator_name)
    row = cursor.fetchone()
    if row:
        logger.debug(f"Found existing OperatorID: {row[0]} for OperatorName: {operator_name}")
        return row[0]
    try:
        ...
        logger.debug(f"Inserted new OperatorID: {operator_id} for OperatorName: {operator_name}")
        return operator_id
    except Exception as e:
        logger.error(f"Failed to insert/select OperatorID for OperatorName '{operator_name}': {e}")
        return None
...
for idx, row in df.iterrows():
    try:
        ...
        if not product_name or pd.isna(product_name):
            logger.warning(f"Row {idx+1} skipped: ProductName (FORMULATION) is missing.")
            continue

        # Insert or get IDs
        can_id = get_or_insert_can(line_name)
        if can_id is None:
            logger.warning(f"Row {idx+1} skipped: Could not get or insert CanID for LineName '{line_name}'.")
            continue
        product_id = get_or_insert_product(product_name, product_code)
        if product_id is None:
            logger.warning(f"Row {idx+1} skipped: Could not get or insert ProductID for ProductName '{product_name}'.")
            continue
        operator_id = get_or_insert_operator(operator_name)
        if operator_id is None:
            logger.warning(f"Row {idx+1} skipped: Could not get or insert OperatorID for OperatorName '{operator_name}'.")
            continue
